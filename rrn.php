...
$Comp = $_GET["Comp"];
$www = string;
$result = mysql_query("SELECT * FROM  `newmemberdata` WHERE  `Competencycategory` LIKE  '$Comp'");
...
if (mysql_num_rows($result)<3){
	...
	for($s=0;$s<10;$s++){
		...
		$addNew = "INSERT INTO  `cbrdatabase`.`newmemberdata` (`ID` ,`Competencycategory` ,`Name` ,`D1` ,`D2` ,`D3` ,`D4` ,`memberid`)VALUES (NULL ,  '$Comp',  '$www',  '$crycry[0]',  '$crycry[1]',  '$crycry[2]',  '$crycry[3]',  '');";
		mysql_query($addNew);
